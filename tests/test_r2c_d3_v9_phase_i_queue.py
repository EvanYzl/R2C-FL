...
from r2c_baselines import r2c_d3_v9_phase_i_queue as queue


def test_phase_i_manifest_is_exact_three_treatment_validation_matrix() -> None:
    ...
    for job in jobs:
        ...
        assert job["variant"] == "telemetry_quarantine_v9"
        assert job["method_config"]["r2c_v2_audit_replay"] is False
        assert job["method_config"]["r2c_v7_trigger_deployment_beta"] == 1.0


def test_v9_changes_only_frozen_detector_and_quarantine_fields_from_control() -> None:
    ...
    assert treatment["r2c_v7_trigger_deployment_beta"] == 1.0
...
def test_phase_i_gate_requires_all_preregistered_conditions() -> None:
    ...
    failed.loc[
        ...
        & (failed["variant"] == "telemetry_quarantine_v9"),
        "trigger_rounds_json",
    ] = "[301]"
...
def test_phase_i_tta_requires_mean_two_percent_and_each_five_percent() -> None:
    failed = _passing_frame()
    failed.loc[
        (failed["seed"] == queue.SEEDS[0])
        & (failed["variant"] == "telemetry_quarantine_v9"),
    ...
    for seed in queue.SEEDS:
        mask = (failed["seed"] == seed) & (failed["variant"] == "telemetry_quarantine_v9")
        control_tta = float(
...
def test_phase_i_rejects_a_broken_quarantine_contract() -> None:
    failed = _passing_frame()
    mask = (failed["seed"] == queue.SEEDS[0]) & (
        failed["variant"] == "telemetry_quarantine_v9"
    )
    failed.loc[mask, "trigger_deployment_hash_held"] = False
    _, gates = queue._evaluate_pairs(failed)
    assert gates["trigger_deployment_hash_held"] is False
    assert all(
        value for key, value in gates.items() if key != "trigger_deployment_hash_held"
    )
