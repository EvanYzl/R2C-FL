...
from . import r2c_d3_v14_formal_confirmation_queue as formal
...
MANIFEST_PATH = QUEUE_ROOT / "r2c_v14_all_datasets_manifest.json"
STATE_PATH = QUEUE_ROOT / "r2c_v14_all_datasets_queue_state.json"
EVENTS_PATH = QUEUE_ROOT / "r2c_v14_all_datasets_scheduler_events.parquet"
RESULT_PATH = QUEUE_ROOT / "r2c_v14_all_datasets_result.json"
RUNS_PATH = QUEUE_ROOT / "r2c_v14_all_datasets_runs.parquet"
RUNS_CSV_PATH = QUEUE_ROOT / "r2c_v14_all_datasets_runs.csv"
...
def _implementation_hashes() -> dict[str, str]:
    ...
    for name in (
        ...
        "test_r2c_v14_all_datasets_queue.py",
    ):
...
def _m3_context() -> dict[str, Any]:
    ...
    if any(not path.exists() for path in required):
        raise RuntimeError("v14 M3 did not authorize the all-dataset matrix")
    ...
    formal._assert_immutable_authority(state, manifest)
    if (
    ...
    if (
        ...
        raise RuntimeError("v14 M3 immutable lineage mismatch")
    candidate_id = str(manifest.get("selected_candidate", {}).get("candidate_id", ""))
    if candidate_id not in formal.development.screen.CANDIDATES:
        raise RuntimeError("v14 M3 selected candidate drift")
    audit_logs: list[Path] = []
    run_dirs: list[Path] = []
    for job in manifest["jobs"]:
        if job.get("status") != "completed" or not job.get("actual_run_id"):
            raise RuntimeError("v14 M3 contains an incomplete formal source job")
        run_id = str(job["actual_run_id"])
        run_dir = RUN_ROOT / run_id
        candidates = (
            QUEUE_ROOT / "worker_logs" / f"{run_id}.m3.reconcile.{job['method_version']}.audit.log",
            QUEUE_ROOT / "worker_logs" / f"{run_id}.m3.{job['method_version']}.audit.log",
        )
        audit_log = next(
            (
                path
                ...
                if path.is_file()
                and formal.development._audit_log_passed(job, run_id, path)
            ),
            None,
        )
        if audit_log is None or not (run_dir / "_SUCCESS.json").exists():
            raise RuntimeError("v14 M3 formal audit/success evidence is missing")
...
def _v11_dataset_context() -> dict[str, dict[str, Any]]:
    ...
    for dataset_id in DATASET_ORDER:
        ...
        if (
            ...
            or any(int(job.get("rounds", -1)) != int(DATASETS[dataset_id].round_budget) for job in pair)
        ):
            raise RuntimeError(f"v11 matched source drift for {dataset_id}")
        config = dict(pair[0]["method_config"])
        if (
            config.get("r2c_protocol_version") != formal.V11_PROTOCOL_VERSION
...
def _dataset_method_config(
    ...
    config = formal._v14_config(candidate_id)
    ...
    if (
        ...
        or config.get("r2c_v14_plan_id") != PLAN_ID
        ...
        raise RuntimeError(f"v14 all-dataset configuration drift for {dataset_id}")
    return config
...
def _asset_paths() -> list[Path]:
    ...
    for dataset_id in DATASET_ORDER:
        ...
        paths.extend((partition_asset_path(dataset_id, SEED), partition_meta_path(dataset_id, SEED)))
        for scenario in SCENARIO_ORDER:
            paths.extend(
                (
...
def _ensure_assets() -> dict[str, str]:
    ...
    if any(not path.exists() for path in paths):
        raise RuntimeError("v14 all-dataset asset preparation is incomplete")
...
def _existing_asset_hashes() -> dict[str, str]:
    paths = _asset_paths()
    if any(not path.exists() for path in paths):
        raise RuntimeError("v14 all-dataset frozen asset is missing")
    return {str(path.relative_to(PROJECT_ROOT)): sha256_file(path) for path in paths}
...
def _source_lineage(context: dict[str, Any]) -> dict[str, str]:
    ...
    if any(not path.exists() for path in paths):
        raise RuntimeError("v14 all-dataset source lineage is incomplete")
    return {str(path.relative_to(PROJECT_ROOT)): sha256_file(path) for path in paths}
...
def _job(
    ...
    if dataset_id not in DATASET_ORDER or scenario not in SCENARIO_ORDER:
        raise ValueError("job is outside immutable v14 M4 dataset/scenario order")
    candidate_id = str(context["candidate_id"])
    rounds = int(DATASETS[dataset_id].round_budget)
    run_id = f"A-R2C-{dataset_id}-{scenario}-V14M4-{candidate_id}-s{SEED}"
    return {
        "job_id": run_id,
        "base_run_id": run_id,
        "stage": "v14_all_datasets_locked_formal_engineering_reevaluation",
        "mode": "formal",
        "method_id": "R2C-FL",
        "method_version": "v14",
        ...
        "block_id": "A-R2C-V14-M4-ALL-DATASETS",
        "seed": SEED,
        "partition_seed": SEED,
        "trace_seed": SEED,
...
def build_manifest(persist: bool = True, force: bool = False) -> dict[str, Any]:
    # M3 is the only authority allowed to unlock other-dataset assets or jobs.
    context = _m3_context()
    ...
    if force and MANIFEST_PATH.exists():
        ...
        if any(
            ...
            raise RuntimeError("Refusing to rebuild a started v14 all-dataset manifest")
    ...
    if [(job["dataset_id"], job["scenario_id"]) for job in jobs] != expected_order:
        raise AssertionError("v14 M4 order must be D1--D4 x S0/S4")
    manifest: dict[str, Any] = {
        "schema_version": "1.0.0",
        "created_utc": utc_now(),
        "scope": "v14_D1_D4_S0_S4_seed20260811_locked_engineering_reevaluation",
        "plan_id": PLAN_ID,
        ...
        "formal_interpretation": "gated_engineering_reevaluation_after_untouched_D3_confirmation",
        "diagnostic_metrics": (
            "last50 accuracy; LQA20; algorithm TTA; Recovery-deficit AUC20; TRM20"
        ),
    ...
    if persist:
        ...
        immutable_path = QUEUE_ROOT / f"r2c_v14_all_datasets_manifest_{stamp}.json"
...
def _assert_immutable_authority(state: dict[str, Any], manifest: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError("v14 M4 immutable manifest hash mismatch")
    immutable = json.loads(immutable_path.read_text(encoding="utf-8"))
    if _frozen_spec(immutable) != _frozen_spec(manifest):
        raise RuntimeError("v14 M4 active/immutable frozen specifications diverged")
...
def _assert_frozen_manifest(manifest: dict[str, Any]) -> None:
    ...
    if (
        manifest.get("scope")
        != "v14_D1_D4_S0_S4_seed20260811_locked_engineering_reevaluation"
        ...
        or manifest.get("round_budgets")
        != {dataset_id: int(DATASETS[dataset_id].round_budget) for dataset_id in DATASET_ORDER}
        ...
        raise RuntimeError("v14 all-dataset manifest scope drift")
    if manifest.get("source_lineage") != _source_lineage(context):
        raise RuntimeError("v14 all-dataset source lineage drift")
    if manifest.get("asset_hashes") != _existing_asset_hashes():
        raise RuntimeError("v14 all-dataset asset drift")
    if manifest.get("implementation_hashes") != _implementation_hashes():
        raise RuntimeError("v14 all-dataset implementation drift")
    if manifest.get("frozen_spec_hash") != config_hash(_frozen_spec(manifest)):
        raise RuntimeError("v14 all-dataset frozen specification drift")
    jobs = list(manifest.get("jobs", []))
    expected_order = [
        (dataset_id, scenario)
        for dataset_id in DATASET_ORDER
    ...
    if len(jobs) != 8 or [job["job_id"] for job in jobs] != manifest.get("job_order"):
        raise RuntimeError("v14 all-dataset matrix/order drift")
    candidate_id = str(context["candidate_id"])
    for job, (dataset_id, scenario) in zip(jobs, expected_order):
        if (
            ...
            or job.get("method_config") != _dataset_method_config(dataset_id, context, dataset_context)
            or job.get("selected_candidate_id") != candidate_id
            or float(job.get("fast_beta", -1.0)) != FAST_BETA
            or float(job.get("stable_beta", -1.0)) != candidate_stable_beta(candidate_id)
            or int(job.get("warmup_rounds", -1)) != WARMUP_ROUNDS
            or int(job.get("recovery_rounds", -1)) != candidate_recovery_rounds(candidate_id)
            ...
            raise RuntimeError(f"v14 all-dataset job drift: {job.get('job_id')}")
...
def _successful_run_id(job: dict[str, Any]) -> str | None:
    ...
    for path in paths:
        ...
        if (path / "_SUCCESS.json").exists() and (path / "result.json").exists():
            audit_log = QUEUE_ROOT / "worker_logs" / f"{path.name}.m4.reconcile.v14.audit.log"
            audit = formal.development._audit_run(job, path.name, audit_log)
            if (
                audit.returncode == 0
                and formal.development._audit_log_passed(job, path.name, audit_log)
            ):
                return path.name
    return None
...
def _reconcile_successes(manifest: dict[str, Any], events: list[dict[str, Any]]) -> int:
    ...
    for job in manifest["jobs"]:
        ...
        if run_id is not None and (
            ...
            _event(events, job, "reconciled_completed", reason="existing_success_output_and_v14_audit")
            changed += 1
    return changed


def _audit_v14_contract(
    rounds: pd.DataFrame,
    scenario: str,
    candidate_id: str,
    ...
    elif scenario == "S4" and event_round is not None:
        expected_trigger = [event_round]
        expected_recovery = list(
            range(event_round + 1, event_round + candidate_recovery_rounds(candidate_id) + 1)
        )
    else:
        raise RuntimeError(f"unexpected v14 all-dataset scenario/event: {run_id}")
    expected_warmup = list(range(1, min(WARMUP_ROUNDS, budget) + 1))
    expected_fast_rounds = set(expected_warmup) | set(expected_recovery)
    ...
    if (
        ...
        or warmup_actual != expected_warmup
        or actual_fast_rounds != expected_fast_rounds
        or not rounds["deployment_cmtr_state_server_only"].astype(bool).all()
        or forbidden_any
    ):
        raise RuntimeError(f"v14 all-dataset CMTR causal contract mismatch: {run_id}")
    if event_round is not None and not np.isclose(
        rounds.loc[rounds["round"] == event_round, "selected_deployment_beta"].iloc[0],
        candidate_stable_beta(candidate_id),
        rtol=0.0,
        atol=0.0,
    ):
        raise RuntimeError(f"v14 all-dataset trigger hold deployment mismatch: {run_id}")


def _time_to_accuracy(rounds: pd.DataFrame, target: float) -> tuple[int | None, float | None]:
    reached = rounds.loc[rounds["test_accuracy"].astype(float) >= target].sort_values("round")
    if reached.empty:
        return None, None
...
def _run_metrics(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError(f"v14 all-dataset run contract mismatch: {run_id}")
    scenario = str(job["scenario_id"])
    event_rows = rounds.loc[pd.to_numeric(rounds["event_offset_round"], errors="coerce") == 0]
    event_round: int | None = None
    if scenario == "S4":
        if len(event_rows) != 1:
            raise RuntimeError(f"v14 all-dataset S4 event mismatch: {run_id}")
        event_round = int(event_rows.iloc[0]["round"])
        if event_round != budget // 2:
            raise RuntimeError(f"v14 all-dataset S4 event boundary drift: {run_id}")
    elif scenario != "S0":
        raise RuntimeError(f"unexpected v14 all-dataset scenario: {scenario}")
    _audit_v14_contract(
        rounds,
        scenario,
        str(job["selected_candidate_id"]),
        event_round,
        budget,
        run_id,
    )
    tta_round, tta_s = _time_to_accuracy(rounds, float(job["target_accuracy"]))
    lqa20: float | None = None
    ...
    if event_round is not None:
        ...
        if not bool(recovery["recovery_auc20_complete"]):
            raise RuntimeError(f"v14 all-dataset AUC@20 window incomplete: {run_id}")
...
def freeze_result(manifest: dict[str, Any]) -> dict[str, Any]:
    if any(job["status"] != "completed" for job in manifest["jobs"]):
        raise RuntimeError("Cannot freeze an incomplete v14 all-dataset matrix")
    ...
    if list(zip(frame["dataset_id"], frame["scenario_id"])) != expected:
        raise RuntimeError("v14 all-dataset terminal matrix/order mismatch")
    if len(frame) != 8 or not frame["source_kind"].astype(str).eq("REPRODUCED").all():
        raise RuntimeError("v14 all-dataset provenance/count mismatch")
...
def worker() -> dict[str, Any]:
    ...
    for position, job in enumerate(manifest["jobs"]):
        ...
        if int(job.get("attempts", 0)) >= MAX_ATTEMPTS:
            ...
            atomic_json(STATE_PATH, state)
            return state
        resolved = formal.development._resolved(job)
        job.update(
            {
        ...
        with log_path.open("w", encoding="utf-8", newline="\n") as handle:
            process = subprocess.run(
                [sys.executable, "-m", "r2c_baselines.run_v14", "--job", str(job_file)],
        ...
        audit_log = QUEUE_ROOT / "worker_logs" / f"{resolved['run_id']}.m4.v14.audit.log"
        audit = (
            formal.development._audit_run(job, str(resolved["run_id"]), audit_log)
            if success
            else None
        )
        if (
            ...
            and formal.development._audit_log_passed(
                job, str(resolved["run_id"]), audit_log
            )
        ):
