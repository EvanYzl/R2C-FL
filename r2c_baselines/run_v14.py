...
from .r2c_v14 import (
    PROTOCOL_VERSION as R2C_V14_PROTOCOL_VERSION,
    CMTRObservation,
    CausalMultiTimescaleRouter,
    candidate_stable_beta as v14_candidate_stable_beta,
    run_r2c_v14_round,
    validated_candidate_id as validated_v14_candidate_id,
)
...
def _run_manifest_base(
    ...
    return {
        ...
        "checkpoint_serialization_version": (
            ...
            if job["method_id"] == PROPOSED_METHOD
            and method_config.get("r2c_protocol_version") == R2C_V14_PROTOCOL_VERSION
            else
...
def run_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    deployment_cmtr: CausalMultiTimescaleRouter | None = None
    if job["method_id"] == PROPOSED_METHOD and protocol_version in {
        ...
        if protocol_version == R2C_V14_PROTOCOL_VERSION:
            candidate_id = validated_v14_candidate_id(method_config)
            stable_beta = v14_candidate_stable_beta(candidate_id)
            if deployment_betas != (0.9, stable_beta):
                raise ValueError(
                    "R2C-v14 requires exactly the registered fast and stable deployment states"
                )
            if primary_deployment_beta != stable_beta:
                raise ValueError("R2C-v14 configured primary beta must be the stable beta")
        ...
        if protocol_version == R2C_V14_PROTOCOL_VERSION:
            deployment_cmtr = CausalMultiTimescaleRouter.from_config(method_config)
    ...
    try:
        for round_number in range(1, rounds + 1):
            ...
            deployment_cmtr_observation: CMTRObservation | None = (
                deployment_cmtr.step(
                    round_number,
                    telemetry_trigger_detected and not no_drift_quarantine,
                )
                if deployment_cmtr is not None
                else None
            )
            if deployment_cmtr_observation is not None:
                telemetry_response_applied = bool(
                    deployment_cmtr_observation.response_applied
                )
            elif deployment_dare_observation is not None:
                telemetry_response_applied = bool(
            ...
            deployment_quarantine_applied = bool(
                ...
                and (
                    ...
                    or (
                        protocol_version == R2C_V14_PROTOCOL_VERSION
                        and deployment_cmtr_observation is not None
                        and deployment_cmtr_observation.hold_applied
                    )
                )
            )
            ...
            deployment_cmtr_recovery_applied = bool(
                protocol_version == R2C_V14_PROTOCOL_VERSION
                and deployment_cmtr_observation is not None
                and deployment_cmtr_observation.recovery_fast_applied
            )
            deployment_cmtr_warmup_applied = bool(
                protocol_version == R2C_V14_PROTOCOL_VERSION
                and deployment_cmtr_observation is not None
                and deployment_cmtr_observation.warmup_fast_applied
            )
            deployment_cmtr_stable_applied = bool(
                protocol_version == R2C_V14_PROTOCOL_VERSION
                and deployment_cmtr_observation is not None
                and deployment_cmtr_observation.stable_route_applied
            )
            ...
            elif protocol_version == R2C_V14_PROTOCOL_VERSION:
                configured_trigger_deployment_beta = 1.0
            ...
            elif deployment_cmtr_recovery_applied:
                deployment_trigger_action = "cmtr_recovery_fast"
            elif deployment_cmtr_warmup_applied:
                deployment_trigger_action = "cmtr_warmup_fast"
            elif deployment_cmtr_stable_applied:
                deployment_trigger_action = "cmtr_stable"
            else:
            ...
            selected_deployment_beta = primary_deployment_beta
            if deployment_models:
                for beta, deployment_model in deployment_models.items():
                    ...
                    if deployment_cmtr_observation is not None:
                        effective_beta = deployment_cmtr_observation.update_beta_for(beta)
                    else:
                        effective_beta = (
                ...
                if deployment_cmtr_observation is not None:
                    selected_deployment_beta = float(
                        deployment_cmtr_observation.selected_beta
                    )
                    effective_primary_deployment_beta = float(
                        deployment_cmtr_observation.update_beta_for(
                ...
                evaluation_model = deployment_models[float(selected_deployment_beta)]
            ...
            if deployment_models:
                ...
                for beta, deployment_model in deployment_models.items():
                    if beta == selected_deployment_beta:
                    ...
                    deployment_writer.append(
                        {
                            ...
                            "effective_deployment_beta": (
                                deployment_cmtr_observation.update_beta_for(beta)
                                if deployment_cmtr_observation is not None
                                else (
                                    float(deployment_beta_override)
                                    if deployment_beta_override is not None
                            ...
                            "deployment_cmtr_recovery_applied": deployment_cmtr_recovery_applied,
                            "deployment_cmtr_warmup_applied": deployment_cmtr_warmup_applied,
                            "deployment_cmtr_stable_applied": deployment_cmtr_stable_applied,
                            ...
                            "deployment_cmtr_phase": (
                                deployment_cmtr_observation.phase
                                if deployment_cmtr_observation is not None
                                else None
                            ),
                            "deployment_cmtr_candidate_id": (
                                deployment_cmtr_observation.configured_candidate_id
                                if deployment_cmtr_observation is not None
                                else None
                            ),
                            "deployment_cmtr_selected_role": (
                                deployment_cmtr_observation.selected_role
                                if deployment_cmtr_observation is not None
                                else None
                            ),
                            "deployment_cmtr_selected_beta": (
                                deployment_cmtr_observation.selected_beta
                                if deployment_cmtr_observation is not None
                                else None
                            ),
                            ...
                            "is_selected_for_deployment": bool(
                                beta == selected_deployment_beta
                            ),
            ...
            if r2c_result is not None and full_logging:
                if telemetry_shift_observation is not None:
                    ...
                    certificate["deployment_cmtr_recovery_applied"] = (
                        deployment_cmtr_recovery_applied
                    )
                    certificate["deployment_cmtr_warmup_applied"] = (
                        deployment_cmtr_warmup_applied
                    )
                    certificate["deployment_cmtr_stable_applied"] = (
                    ...
                    if deployment_cmtr_observation is not None:
                        certificate.update(deployment_cmtr_observation.audit_fields())
                    ...
                    for row in r2c_result.checkpoint_rows:
                        ...
                        row["deployment_cmtr_recovery_applied"] = (
                            deployment_cmtr_recovery_applied
                        )
                        row["deployment_cmtr_warmup_applied"] = (
                            deployment_cmtr_warmup_applied
                        )
                        row["deployment_cmtr_stable_applied"] = (
                            deployment_cmtr_stable_applied
                        )
                        ...
                        if deployment_cmtr_observation is not None:
                            row.update(deployment_cmtr_observation.audit_fields())
            ...
            if telemetry_shift_observation is not None:
                ...
                if deployment_cmtr_observation is not None:
                    round_row.update(deployment_cmtr_observation.audit_fields())
                round_row.update(
                    {
                        ...
                        "selected_deployment_beta": float(selected_deployment_beta),
                        "selected_deployment_model_hash_after": deployment_hashes_after[
                            format(float(selected_deployment_beta), ".17g")
                        ],
                    }
                )
