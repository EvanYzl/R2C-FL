...
from .r2c_v4_table1_matched_queue import FORMAL_INTERPRETATION
...
def _paths(stem: str, extension: str, stamp: str) -> tuple[Path, Path]:
    return (
        PLOT_ROOT / f"{stem}_{stamp}.{extension}",
        PLOT_ROOT / f"{stem}.{extension}",
    )
...
def _load_jobs() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if payload.get("protocol_version") != PROTOCOL_VERSION:
        raise RuntimeError("Matched Table 1 manifest protocol drift")
    if int(payload.get("formal_seed", -1)) != 20260811:
        raise RuntimeError("Matched Table 1 manifest seed drift")
    jobs = list(payload.get("jobs", []))
    if len(jobs) != 8:
        raise RuntimeError(f"Expected 8 matched v4 formal jobs, found {len(jobs)}")
    if any(job.get("status") != "completed" or not job.get("actual_run_id") for job in jobs):
        raise RuntimeError("All 8 matched v4 formal jobs must complete before finalization")
    for job in jobs:
...
def finalize() -> dict[str, Any]:
    ...
    observed = set(
        combined[["method_id", "dataset_id", "scenario_id"]].itertuples(index=False, name=None)
    )
    if expected != observed or len(combined) != 64 or combined["run_id"].duplicated().any():
        raise RuntimeError("Matched v4 Table 1 run matrix is incomplete or duplicated")
    ...
    outputs: list[str] = []
    outputs += _write_parquet("r2c_v4_table1_run_summary", r2c_summary, stamp)
    outputs += _write_parquet("table1_v4_matched_combined_run_summary", combined, stamp)
    outputs += _write_parquet("table1_v4_matched_combined_values", values, stamp)
    outputs += _write_csv("table1_v4_matched_combined_values", values, stamp)
    outputs += _write_json(
        "r2c_v4_table1_audit_report",
        {"generated_utc": generated, "runs": audit_reports},
        stamp,
    )
    outputs += _write_json(
        "r2c_v4_table1_auc20_windows",
        {"generated_utc": generated, "runs": window_reports},
        stamp,
    )
    outputs += _write_json(
        "r2c_v4_table1_values",
        {
    ...
    outputs += _write_json("r2c_v4_table1_qc_report", qc, stamp)
    result = {
        "status": "completed",
        "qc_complete": True,
        "formal_seed": 20260811,
    ...
    outputs += _write_json("r2c_v4_table1_aggregation_result", result, stamp)
