...
MANIFEST_PATH = QUEUE_ROOT / "r2c_d2_v4_fullval_manifest.json"
STATE_PATH = QUEUE_ROOT / "r2c_d2_v4_fullval_queue_state.json"
EVENTS_PATH = QUEUE_ROOT / "r2c_d2_v4_fullval_scheduler_events.parquet"
RESULT_PATH = QUEUE_ROOT / "r2c_d2_v4_full_validation_result.json"
RUNS_PATH = PLOT_ROOT / "r2c_d2_v4_full_validation_runs.parquet"
RUNS_CSV_PATH = PLOT_ROOT / "r2c_d2_v4_full_validation_runs.csv"
...
def _job(scenario_id: str, config: dict[str, Any], beta: float) -> dict[str, Any]:
    ...
    job_id = f"A-R2C-D2-{scenario_id}-V4-FULLVAL-{label}-s{DEV_SEED}"
    return {
        "job_id": job_id,
        "base_run_id": job_id,
        "stage": "v4_full_validation",
        "mode": "calibration",
        "method_id": "R2C-FL",
        "dataset_id": "D2",
...
def build_manifest(persist: bool = True, force: bool = False) -> dict[str, Any]:
    ...
    if not SELECTION_PATH.exists():
        raise RuntimeError("V4 full validation requires frozen calibration selection")
    selection = json.loads(SELECTION_PATH.read_text(encoding="utf-8"))
    if selection.get("test_labels_used") or selection.get("selection_split") != "validation":
        raise RuntimeError("V4 selection is not validation-only")
    if not selection.get("advance_to_full_validation"):
        raise RuntimeError("V4 calibration did not authorize full validation")
    beta = float(selection["winner_beta"])
    config = dict(selection["winner_method_config"])
    if config.get("r2c_protocol_version") != PROTOCOL_VERSION:
        raise RuntimeError("V4 protocol drift in calibration winner")
    if config.get("r2c_v4_deployment_ema_betas") != [beta]:
        raise RuntimeError("V4 full validation must use one frozen beta")
    jobs = [_job(scenario_id, config, beta) for scenario_id in ("S0", "S4")]
    manifest = {
        "schema_version": "1.0.0",
        "created_utc": utc_now(),
        "scope": "D2_only_v4_fixed_beta_full_validation",
        ...
        "selection_hash": config_hash(selection),
        "implementation_hashes": _implementation_hashes(),
        "jobs": jobs,
    }
    if persist:
        stamp = utc_now().replace(":", "").replace("-", "")
        atomic_json(QUEUE_ROOT / f"r2c_d2_v4_fullval_manifest_{stamp}.json", manifest)
...
def freeze_result(manifest: dict[str, Any]) -> dict[str, Any]:
    if any(job["status"] != "completed" for job in manifest["jobs"]):
        raise RuntimeError("Cannot freeze incomplete v4 full validation")
    ...
    for job in manifest["jobs"]:
        ...
        if job["scenario_id"] == "S4" and auc is None:
            raise RuntimeError("V4 full-validation S4 lacks complete AUC@20")
