...
from .r2c_d3_v7_phase_e_queue import (
    _audit_run,
    _treatment_config,
    _verified_chunked_table,
    _verify_source_control,
)
...
PHASE_E_RESULT_PATH = QUEUE_ROOT / "r2c_d3_v7_phase_e_result.json"
PHASE_E_MANIFEST_PATH = (
    QUEUE_ROOT / "r2c_d3_v7_phase_e_manifest_20260817T090307.976624Z.json"
)
PHASE_E_RUNS_PATH = PLOT_ROOT / "r2c_d3_v7_phase_e_runs.parquet"
PHASE_E_PAIRS_PATH = PLOT_ROOT / "r2c_d3_v7_phase_e_pairs.parquet"

MANIFEST_PATH = QUEUE_ROOT / "r2c_d3_v7_phase_f_manifest.json"
STATE_PATH = QUEUE_ROOT / "r2c_d3_v7_phase_f_queue_state.json"
EVENTS_PATH = QUEUE_ROOT / "r2c_d3_v7_phase_f_scheduler_events.parquet"
RESULT_PATH = QUEUE_ROOT / "r2c_d3_v7_phase_f_result.json"
RUNS_PATH = PLOT_ROOT / "r2c_d3_v7_phase_f_runs.parquet"
RUNS_CSV_PATH = PLOT_ROOT / "r2c_d3_v7_phase_f_runs.csv"
...
def _verify_phase_e_result() -> dict[str, Any]:
    if not PHASE_E_RESULT_PATH.exists():
        raise RuntimeError("D3 v7 Phase E result does not exist")
    result = json.loads(PHASE_E_RESULT_PATH.read_text(encoding="utf-8"))
    if (
        not bool(result.get("phase_e_passed"))
        or not bool(result.get("phase_f_authorized"))
        ...
        raise RuntimeError("D3 v7 Phase E did not authorize Phase F")
    state = json.loads(
        (QUEUE_ROOT / "r2c_d3_v7_phase_e_queue_state.json").read_text(encoding="utf-8")
    )
    if state.get("status") != "phase_e_completed_phase_f_authorized":
        raise RuntimeError("D3 v7 Phase E terminal state does not authorize Phase F")
    return result
...
def _frozen_config() -> dict[str, Any]:
    return _treatment_config(_verify_source_control(run_audit=False))


def _job(scenario: str, config: dict[str, Any]) -> dict[str, Any]:
    run_id = f"A-R2C-D3-{scenario}-V7F-SYNC-s{SEED}"
    return {
        "job_id": run_id,
        "base_run_id": run_id,
        "stage": "d3_v7_phase_f_full_validation_authorization",
        "mode": "calibration",
        "method_id": "R2C-FL",
...
def build_manifest(persist: bool = True, force: bool = False) -> dict[str, Any]:
    ...
    if persist:
        ...
        atomic_json(QUEUE_ROOT / f"r2c_d3_v7_phase_f_manifest_{stamp}.json", manifest)
...
def _assert_frozen_manifest(manifest: dict[str, Any]) -> None:
    if manifest.get("formal_test_access") or manifest.get("other_dataset_access"):
        raise RuntimeError("D3 v7 Phase F attempted prohibited access")
    if manifest.get("implementation_hashes") != _implementation_hashes():
        raise RuntimeError("Implementation hashes changed after D3 v7 Phase F freeze")
    if manifest.get("source_hashes") != _source_hashes():
        raise RuntimeError("Source hashes changed after D3 v7 Phase F freeze")
    if manifest.get("asset_hashes") != _ensure_assets():
        raise RuntimeError("Asset hashes changed after D3 v7 Phase F freeze")
    jobs = manifest.get("jobs", [])
    if len(jobs) != 2 or [job["job_id"] for job in jobs] != manifest.get("job_order"):
        raise RuntimeError("D3 v7 Phase F matrix drift")
    config = _frozen_config()
    for job, scenario in zip(jobs, SCENARIOS):
        if (
            ...
            raise RuntimeError(f"D3 v7 Phase F protocol drift in {job['job_id']}")
...
def _run_metrics(run_id: str, scenario: str) -> dict[str, Any]:
    ...
    if rounds["round"].astype(int).tolist() != list(range(1, ROUNDS + 1)):
        raise RuntimeError(f"Incomplete Phase F trajectory: {run_id}")
    if (
        ...
        raise RuntimeError(f"Phase F source/seed/split mismatch: {run_id}")
    ...
    if scenario == "S4":
        ...
        if len(events) != 1 or int(events.iloc[0]["round"]) != EVENT_ROUND:
            raise RuntimeError(f"Phase F event drift: {run_id}")
        ...
        if not direct["recovery_auc20_complete"]:
            raise RuntimeError(f"Phase F lacks strict AUC@20: {run_id}")
        ...
        if stored is None or abs(float(stored) - auc) > 1.0e-15:
            raise RuntimeError(f"Phase F stored/direct AUC mismatch: {run_id}")
    ...
    if abs(float(result["last50_accuracy"]) - direct_last50) > 1.0e-15:
        raise RuntimeError(f"Phase F Last50 mismatch: {run_id}")
...
def freeze_result(manifest: dict[str, Any]) -> dict[str, Any]:
    if any(job["status"] != "completed" for job in manifest["jobs"]):
        raise RuntimeError("Cannot freeze incomplete D3 v7 Phase F")
    rows = [
...
def worker() -> dict[str, Any]:
    ...
    state.update(
        {
            "status": (
                "phase_f_completed_formal_authorized"
                if result["formal_authorized"]
                else "phase_f_completed_formal_not_authorized"
            ),
            "current_job_id": None,
